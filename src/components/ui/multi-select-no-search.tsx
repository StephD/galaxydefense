...
interface MultiSelectNoSearchProps {
  options: Option[];
  selected: string[];
...
export function MultiSelectNoSearch({
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {/* Dropdown menu - without search box */}
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-background rounded-md border border-input shadow-md">
          <div className="max-h-[200px] overflow-auto p-1">
            {options.length === 0 ? (
              <div className="py-2 px-2 text-sm text-center text-muted-foreground">No options available</div>
            ) : (
              options.map(option => (
                <div
                  key={option.value}
